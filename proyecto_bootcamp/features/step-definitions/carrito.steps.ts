...
Given(/^Yo agrego productos al carrito$/, async () => {
    ...
    await browser.pause(2000);
    await inicio_sesionTask.btnCompraDeNuevo.click();
...
Then(/^El carrito debe contener al menos 3 productos$/, async () => {
 ...
 Then(/^El subtotal debe corresponder a la suma de los 3 productos$/, async () => {
     const { subtotal } = await carritoTask.subtotal;
     const subtotalPrice = parseFloat(subtotal.replace('US$', '').trim());
     ...
     expect(Number(subtotalPrice)).toEqual(calculatedSubtotal);
 });
